...
- (void)createAdBannerView {
	...
	NSString *post = [NSString stringWithFormat:@"&action=%@&appId=%@&deviceType=%@&orientationType=%@&sdkver=%@&model=%@&deviceid=%@&systemversion=%@&locale=%@&language=%@&top=%d&installkey=%@&macaddress=%@&iphone5=%@"
					  ,@"httpsShowAd",self.key,self.deviceType,self.OrientationType,OPENCLIKSDKVER,model,deviceid,systemVersion,locale,language,top,installkey,[self macaddress],iphone5];
...
{ 
	...
	if ([response count] <2 ) {
        ...
        topadRequestURL = [keyTop substringFromIndex:range.location+1];
    ...
    NSURLRequest *requestTopURL = [NSURLRequest requestWithURL:[NSURL URLWithString:topadRequestURL]];
     ...
     //add for local apple image support
        
    NSString *resourcePath = [[NSBundle mainBundle] resourcePath];
    resourcePath = [resourcePath stringByReplacingOccurrencesOfString:@"/" withString:@"//"];
    resourcePath = [resourcePath stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
    
    [_adBannerView loadHTMLString:HTMLData baseURL:
     [NSURL URLWithString:
      [NSString stringWithFormat:@"file:/%@//",resourcePath]
      ]];
    
   
    returnData = [ NSURLConnection sendSynchronousRequest:requestFullURL returningResponse: nil error: nil ];
    HTMLData = [[NSString alloc] initWithData:returnData encoding:NSUTF8StringEncoding];
    NSLog(@"%@",HTMLData);
    ...
    [_adFullView loadHTMLString:HTMLData baseURL:
     [NSURL URLWithString:
      [NSString stringWithFormat:@"file:/%@//",resourcePath]
      ]];
    
    
    returnData = [ NSURLConnection sendSynchronousRequest:requestTopURL returningResponse: nil error: nil ];
    HTMLData = [[NSString alloc] initWithData:returnData encoding:NSUTF8StringEncoding];
    ...
    [_adTopView loadHTMLString:HTMLData baseURL:
     [NSURL URLWithString:
      [NSString stringWithFormat:@"file:/%@//",resourcePath]
      ]];
    */
    ...
    if(![_adBannerView isLoading])
        [_adBannerView loadHTMLString:adRequestURL baseURL:[NSURL URLWithString:[NSString stringWithFormat:@"file:/%@//",resourcePath]]];
    if(![_adFullView isLoading])
        [_adFullView loadHTMLString:fulladRequestURL baseURL:[NSURL URLWithString:[NSString stringWithFormat:@"file:/%@//",resourcePath]]];
    if(![_adTopView isLoading])
       [_adTopView loadHTMLString:topadRequestURL baseURL:[NSURL URLWithString:[NSString stringWithFormat:@"file:/%@//",resourcePath]]];
	_adBannerView.delegate = self;
	_adFullView.delegate = self;
...
-(void)touchView:(UITouch*)touch
{
	...
	if (touchPoint.x>0 && touchPoint.x<touchX && touchPoint.y>0 && touchPoint.y<touchY) {
        ...
        NSString *deviceid=[self uniqueGlobalDeviceIdentifier ];
        NSString *systemVersion = [[UIDevice currentDevice] systemVersion];
		NSString *post = [NSString stringWithFormat:@"&action=%@&appId=%@&adId=%@&deviceType=%@&orientationType=%@&fullScreen=%@&locale=%@&language=%@&securekey=%@&model=%@&deviceid=%@&installkey=%@&macaddress=%@&sdkver=%@&systemversion=%@",@"httpsClickAd",self.key,self.adId,self.deviceType,self.OrientationType,self.adType,locale,language,self.secureKey,model,deviceid,installkey,[self macaddress],OPENCLIKSDKVER,systemVersion];
