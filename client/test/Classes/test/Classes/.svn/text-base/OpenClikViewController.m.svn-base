...
{	
    ...
    adStop = NO;
    [self performSelector:@selector(showAd) withObject:nil afterDelay:0.1];
}
-(void)showAd
{
    if (adStop) 
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation {
    ...
    if (UIInterfaceOrientationIsLandscape(tadClass.orientation)) {
        return (interfaceOrientation==UIInterfaceOrientationLandscapeLeft || interfaceOrientation==UIInterfaceOrientationLandscapeRight);
    }
    else
    {
        return (interfaceOrientation==tadClass.orientation);
    }
...
-(void)setMaskLayer;
{
	...
	//UIImageView *icon = [[UIImage alloc] init]; ;
	//icon.image = image;
