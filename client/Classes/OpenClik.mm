...
-(id)initWithKey:(NSString*)key
{
	...
	if (self == [super init]) {
		...
		NSLog(@"openclik init width is %i, height is %i",width,height);
	}
	return self;
}
...
- (void)createAdBannerView {
    ...
    if(version >=6.0)
    {
        ...
        verdorId = [identifierForVendor UUIDString];
    }
	...
	NSString *post = [NSString stringWithFormat:@"&action=%@&appId=%@&deviceType=%@&orientationType=%@&sdkver=%@&model=%@&deviceid=%@&systemversion=%@&locale=%@&language=%@&top=%d&installkey=%@&macaddress=%@&iphone5=%@&network=%@&availableStorage=%llu&totalStorage=%f&identifierForVendor=%@&width=%i&height=%i"
					  ,@"httpsShowAd",self.key,self.deviceType,self.OrientationType,OPENCLIKSDKVER,model,deviceid,systemVersion,locale,language,top,installkey,deviceid,iphone5,ocnetworkType,freeSpace(),getTotalDiskSpaceInBytes(),verdorId,width,height];
...
- (void)fixupAdView:(UIInterfaceOrientation)toInterfaceOrientation {
	...
	{        
		...
		if (_adFullViewIsVisible && _webFullViewIsReady)
		{
            ...
            NSLog(@"%f,%f,%f,%f",adFullViewFrame.origin.x,adFullViewFrame.origin.y,adFullViewFrame.size.width,adFullViewFrame.size.height);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	NSLog(@" full ad is %@",self.fulladRequestURL);
}
-(void)loadWebView
{
...
-(void)touchView:(UITouch*)touch touchView:(UIView*)view
{
    ...
    NSLog(@"touch x is %f,touch y is %f",touchPoint.x,touchPoint.y);
	...
	if ( (touchPoint.y>0 && touchPoint.y-(touchY+topSize)<0  && view ==_adFullView)
        || (touchPoint.y-(topSize+adSize)>0)  )
    {
