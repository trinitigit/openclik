...
- (void)fixupAdView:(UIInterfaceOrientation)toInterfaceOrientation {
	...
	{        
		...
		} else 
		{
			adFullViewFrame.size.width = [self getFullWidth:toInterfaceOrientation];
			adFullViewFrame.size.height = [self getFullHeight:toInterfaceOrientation];
			adFullViewFrame.origin.y = [self getFullHeight:toInterfaceOrientation];
			adFullViewFrame.origin.x = 0;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	...
	if ([response count] <2 ) {
		...
		//self.adRequestURL = [response objectAtIndex:0];
		self.fulladRequestURL = [response objectAtIndex:0];
		self.appStoreURL = [response objectAtIndex:1];
		self.adId = [response objectAtIndex:2];
        self.adAppId = [response objectAtIndex:3];
        NSString *key = [response objectAtIndex:4];
        self.appUrl = [response objectAtIndex:5];
        self.award=[response objectAtIndex:6];
        [[OpenClikViewController getInstance] hide];
        [OpenClikViewController getInstance].view.hidden = YES;
        return ;
        ...
        self.appUrl = [response objectAtIndex:5];
        self.award=[response objectAtIndex:6];
        int widthRatio = [response objectAtIndex:7];
        [[OpenClikViewController getInstance] hide];
        [OpenClikViewController getInstance].view.hidden = YES;
        return ;
	}
        ...
        self.adId = [response objectAtIndex:2];
        self.adAppId = [response objectAtIndex:3];
        self.secureKey = [response objectAtIndex:4];
        self.appUrl = [response objectAtIndex:5];
        self.award=[response objectAtIndex:6];
        int widthRatio = [response objectAtIndex:7];
        int heightRatio = [response objectAtIndex:8];
    }
        ...
        int widthRatio = [response objectAtIndex:7];
        int heightRatio = [response objectAtIndex:8];
        int showTimes = [response objectAtIndex:9];
    }
...
-(void)loadWebView
{
    ...
    NSLog(@"icon ad url is %@",self.fulladRequestURL);
...
-(void)touchView:(UITouch*)touch touchView:(UIView*)view
{
    ...
    CGRect bounds = [[UIScreen mainScreen] bounds];
    touchX = (int)bounds.size.width-touchX;
